``` {r child_230_data_wrangling, echo=FALSE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
  ...
  ##########################################################################################################################################
  # DATA 
  data_30a_trucks <- data_00_base_mod %>%
  ...
  gg_30a_trucks <- ggplot(data_30a_trucks) + 
    geom_line(
      aes(x = crossing_date
          ,y=crossings
    ...
    scale_y_continuous(name = "US Inbound Border Crossings (Trucks)",
                       # breaks = scales::breaks_extended(8),
                       # labels = scales::label_number()  
                       labels = unit_format(unit = "M", scale = 1e-6)
                       ,breaks = seq(0, 1000000, by = 250000)
                       ,minor_breaks = seq(0, 1000000, by = 125000)
                       ,limits= c(0,1000000) 
    ) +
    ...
    labs(
      ...
      title = 'US Inbound Border Crossings by Country by Month (Trucks)',
      color="Transportation Mode",
      # subtitle = 'Color indicates the Mode of Transportation',
      caption = 'Fig 8: Monthly Border Crossings for Trucks by Country'
    )
```